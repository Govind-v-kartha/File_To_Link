...
class OFAViT(nn.Module):
    ...
    def __init__(self, img_size=512, patch_size=16, drop_rate=0.,
        ...
        self.conv0 = nn.Sequential(
            ...
            nn.GroupNorm(32, 512),
            nn.GELU(),
            nn.ConvTranspose2d(512, 256, 32, 32),  # 2048, 16, 16
            nn.Dropout(0.5)
        )
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.GroupNorm(32, 512),
            nn.GELU(),
            nn.ConvTranspose2d(512, 512, 16, 16),  # 2048, 16, 16
            nn.Dropout(0.5)
        )
        ...
        self.conv2 = nn.Sequential(
            ...
            nn.GELU(),
            nn.ConvTranspose2d(1024, 1024, 8, 8),  # 2048, 16, 16
            nn.Dropout(0.5)
        )
        ...
        self.conv3 = nn.Sequential(
            ...
            nn.GELU(),
            nn.ConvTranspose2d(2048, 2048, 4, 4),  # 2048, 16, 16
            nn.Dropout(0.5)
            # 2048, 16, 16
        )
    ...
    def forward(self, x, wave_list):
        ...
        m = {}
        m[0] = self.conv0(seg1)  # 256,112,112
        # print(m[0].shape)

        m[1] = self.conv1(seg1)  # 512,56,56
        # print(m[1].shape)

        m[2] = self.conv2(seg1)  # 1024,28,28
        # print(m[2].shape)

        m[3] = self.conv3(seg1)  # 2048,14,14
        # print(m[3].shape)
...
def vit_base_patch16_32(**kwargs):
    model = OFAViT(
        patch_size=32, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, resize=32,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
...
if __name__ == '__main__':
    ...
    checkpoint_model = torch.load('DOFA_ViT_base_e100.pth', weights_only=True)
    vit_model = vit_base_patch16_32(num_classes=10)
    state_dict = vit_model.state_dict()
    msg = vit_model.load_state_dict(checkpoint_model, strict=False)
    print(msg)
    vit_model = vit_model.cuda()
    # vit_model = vit_base_patch16(num_classes=10).cuda()
    s2_img = torch.randn(2, 3, 512, 512).cuda()
